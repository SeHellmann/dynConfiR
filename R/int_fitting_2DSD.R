fitting2DSD <- function(df, nConds, nRatings, fixed, sym_thetas,
  ...
  get_start_thetas <- function(paramRow) {
    ...
    MRT_corr <- (subset(MRT, correct==1)$rt-paramRow['t0']-paramRow['st0']/2)
    MRT_false <- (subset(MRT, correct==0)$rt-paramRow['t0']-paramRow['st0']/2)
    ...
    Mconf_false = -paramRow['tau']*(-paramRow['a']*paramRow['z']*paramRow['sv']^2+v)/
    ...
    for (i in 1:length(conf_probs)) {
      thetas[i] <- optimize(function(conf) (mixcdf(conf)-conf_probs[i])^2,
                            lower=min(Mconf_false[nConds],Mconf_corr[nConds])- 4*VRconf,
                            upper=max(Mconf_false[nConds],Mconf_corr[nConds])+ 4*VRconf)$minimum
    }
