#' Functions for predicting several sequential sampling confidence models in parallel
#'
...
#'
#' @param paramDf a dataframe with rows for participant and model combination. Columns must include
...
#' @param DistConf NULL or data.frame. For \code{predictRTModels}. A data.frame with participant
...
#'
#'
#' @details The function \code{\link{predictConf}} (called by \code{predictConfModels})
...
predictRTModels <- function(paramDf,
  ...
  if (is.null(minrt)) {
    ...
    if (simult_conf && any(c("2DSD", "dynWEV") %in% models)) {
      pars_diffmodels <- subset(paramDf,model %in% c("2DSD", "dynWEV"))
