/* Density_DDMConf.h - Functions for PDF calculation of the DDMConf model
 *
...
double g_minus_DDMConf (double t);
double g_plus_DDMConf  (double t);

static double integral_t0_g_minus_DDMConf (double t, Parameters *params);
...
static double integrate_z_over_t_DDMConf  (Parameters *params, double a, double b, double step_width);
static double integrate_v_over_zr_DDMConf (Parameters *params, double a, double b, double t, double step_width);


// Main calls
NumericVector density_DDMConf (NumericVector rts, int boundary, int stopon0)
{
    ...
    if (stopon0==1) {
      if (boundary == 1) {
        for (int i = 0; i < length; i++) {
          out[i] =  g_plus_DDMConf(rts[i]);
          if (out[i]==0) break;
          }
        } // Calc upper
      else {
        for (int i = 0; i < length; i++) {
          out[i] = -g_minus_DDMConf(rts[i]);
          if (out[i]==0) break;
          }
        } // Calc lower
    } else {
      if (boundary == 1) { for (int i = 0; i < length; i++) { out[i] =  g_plus_DDMConf(rts[i]);  } } // Calc upper
      else { for (int i = 0; i < length; i++) { out[i] = -g_minus_DDMConf(rts[i]); } } // Calc lower
    }
...
double g_minus_DDMConf(double t)
{
    return integral_t0_g_minus_DDMConf (t - g_Params->t0 - 0.5*g_Params->d, g_Params);
}
...
double g_plus_DDMConf(double t)
{
    ...
    return integral_t0_g_minus_DDMConf (t - new_params.t0 + 0.5*new_params.d, &new_params);
}
...
static double integral_t0_g_minus_DDMConf (double t, Parameters *params)
{
    ...
    {
        if (( 1/(params->th2*params->th2) > (t + .5*params->st0)) ||
            ( 1/(params->th1*params->th1) < (t - .5*params->st0))) {
          return 0.0;
        } else {
          res = integrate_z_over_t_DDMConf(params,
                                           std::max(t - .5*params->st0, 1/(params->th2*params->th2)),
                                           std::min(t + .5*params->st0, 1/(params->th1*params->th1)),
...
static double integral_v_g_minus_DDMConf (double t, double zr, Parameters *params)
{
    ...
    if (N_small < N_large)
    {
        simple = g_minus_small_time_DDMConf(t/(a*a), zr, N_small);
    }
    else
    {
        simple = g_minus_large_time_DDMConf(t/(a*a), zr, N_large);
    }
...
static double integrate_z_over_t_DDMConf (Parameters *params, double a, double b, double step_width)
{
    ...
    {
        result += step * integral_z_g_minus_DDMConf(x, params);
    }
    return result;
}
...
static double integrate_v_over_zr_DDMConf (Parameters *params, double a, double b, double t, double step_width)
{
    ...
    for(x = a+0.5*step; x < b; x += step)
    {
      result += step * integral_v_g_minus_DDMConf (t, x, params);
    }
    return result;
}
