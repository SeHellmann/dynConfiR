fittingdynWEV_formula <- function(DVs, model_matrix,
  ...
  init_grid <- expand.grid(a = c(0.5, 1,1.7, 2.5),                  ### a = distance btw. upper and lower bound \in (0,\infty)]
                           v = c(0.01, 0.5, 2.5),                     ### v = mean drift rate \in (0,\infty)]
                           sv = c(0.01, 0.8, 1.5),                  ### sv = SD of drift rate (normal distr.) \in (0,\infty)]
                           z = mean(DVs$response==1),               ### z = mean start point (bias) \in [0,1]
  ...
  init_grid <- init_grid[complete.cases(init_grid),]
################## HIER WEITERMACHEN ##############################
##################
  ## For each manipulation set some inital values
  if (nConds==1) {
manipulations
    fit_par_names
        init_grid$v <- 1
    ...
    init_grid$svis <- 0.1
    init_grid$w <- 0.5
    init_grid$sigvis <- 0.5
    init_grid$lambda <- 1
  } else {
    init_grid$v <- c(0.5, 1.5, 2.5)
    init_grid$sv <- c(0.01, 0.8, 1.5)
    init_grid$z <- c(0.1, 0.5, 0.9)
    init_grid$sz <- c(0.1, 0.3, 0.5)
    init_grid$tau <- c(0.1, 0.5, 0.9)
    init_grid$svis <- c(0.01, 0.1, 0.5)
    init_grid$w <- c(0.3, 0.5, 0.7)
    init_grid$sigvis <- c(0.01, 0.5, 1)
    init_grid$lambda <- c(0, 0.5, 1, 2)
  }
  ...
  init_thetas <- get_thetas_for_init_grid_dynWEV_simulations(init_grid, DVs, nRatings, simult_conf, fixed, mint0)
  ...
  if (useparallel) {
    ...
    clusterExport(cl, c("DVs", "restr_tau", "mint0",
      ...
      if (useparallel) {
        logL <-
          parApply(cl, inits, MARGIN=1,
                   function(p) try(neglikelihood_dynWEV_free(p, DVs,
        ...
        logL <-
          apply(inits, MARGIN = 1,
                function(p) try(neglikelihood_dynWEV_free(p, DVs, restr_tau, nConds, nRatings, fixed, mint0, simult_conf, sym_thetas, precision),
                                silent = TRUE))
      }
    } else {
      if (useparallel) {
        logL <-
          parApply(cl, inits, MARGIN=1,
                   function(p) try(neglikelihood_dynWEV_bounded(p, DVs, restr_tau, nConds, nRatings, fixed, mint0, simult_conf, sym_thetas, precision),
                                   silent=TRUE))
        #stopCluster(cl)
      } else {
        logL <-
  ...
  if(exists("fit") && is.list(fit)){
    ...
    if (logging==TRUE) {
      ...
      save(logL, DVs, res, file=filename)
    }
  }
  return(res)
}
